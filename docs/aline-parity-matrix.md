...
| CORE-002 | Open/save/save as `.aline` | `Open`, `DumpDataFile`, `UndumpDataFile`, `savepackaline`, `loadpackaline` | partial | `aline-packed-format.md`, `tests/fixtures/aline/README.md` and `npm run test:aline-oracle`: 165 historical assertions, including saved specimens, all 36 drawable types, layout settings, PDB-derived numbering and silent-loss cases; rich Atlas import and lossless `.atlas` round trips remain required |
...
| CORE-028 | Palettes `.alc` et éditeur de palettes | `ReadColour`, `SaveColour`, `CalColours`, `_ApplyCat` | partiel | lecture sûre des 5 palettes, écriture compatible, persistance `.atlas`, application, édition des seuils/couleurs et gradients RGB/HSL testés ; typographie avancée à compléter |
