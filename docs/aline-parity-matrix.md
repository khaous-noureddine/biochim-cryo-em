...
| CORE-002 | Open/save/save as `.aline` | `Open`, `DumpDataFile`, `UndumpDataFile`, `savepackaline`, `loadpackaline` | partial | `aline-packed-format.md` and `npm run test:aline-oracle`: 108 historical assertions, including all 36 drawable types; rich Atlas import fixtures and lossless `.atlas` round trips remain required |
